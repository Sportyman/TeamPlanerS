...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-slate-100 flex flex-col items-center justify-between p-4">
      ...
      <div className="w-full max-w-7xl flex justify-between items-center p-2">
           ...
           {!authInitialized ? (
               ...
               <div className="flex items-center gap-4">
                    ...
                    <button onClick={logout} className="text-slate-400 hover:text-red-500 transition-colors" title="התנתק">
                        <LogOut size={20} />
                    </button>
               </div>
           ) : null}
          ...
          <div className="max-w-4xl w-full space-y-8 md:space-y-12">
            ...
            <div className="bg-white/40 backdrop-blur-md p-6 md:p-10 rounded-[2.5rem] border border-white/50 shadow-xl max-w-2xl mx-auto w-full">
                ...
                {user && (memberships.length > 0 || user.isAdmin) ? (
                    ...
                    // LOGGED OUT VIEW - CLUB SELECTION
                    <div className="space-y-8">
                        <div className="grid grid-cols-2 gap-4 md:gap-6">
                            {clubs.map((club) => (
                                <button
                                    key={club.id}
                                    onClick={() => handleClubSelect(club.id)}
                                    className="group relative bg-white rounded-3xl p-6 shadow-sm hover:shadow-2xl border-2 border-transparent hover:border-brand-500 transition-all duration-500 flex flex-col items-center justify-center gap-4 aspect-square"
                                >
                                    <div className="w-16 h-16 bg-brand-50 text-brand-600 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-transform">
                        ...
                        <div className="text-center">
                            <p className="text-slate-400 text-sm">בחר מועדון כדי להתחבר או להירשם</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="text-center">
               <button 
                 ...
                 className="text-slate-400 hover:text-brand-600 text-xs font-bold flex items-center justify-center gap-2 mx-auto transition-colors py-2 px-4 rounded-full hover:bg-white/50"
               >
                 <Settings size={14} />
                 ניהול מערכת ראשי
               </button>
      ...
      {/* Footer */}
      <footer className="w-full py-6 text-center text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]" dir="ltr">
         TeamPlaner v{APP_VERSION} • Developed by Shay Kalimi
      </footer>
    </div>
  );
};
