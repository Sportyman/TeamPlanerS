...
import { Person, Role, Gender, BoatDefinition, GenderPrefType, GenderPrefLabels, getRoleLabel } from '../../types';
...
export const PersonEditorModal: React.FC<PersonEditorModalProps> = ({ 
    ...
    const [formData, setFormData] = useState<Partial<Person>>(person || {
        gender: Gender.MALE, role: Role.VOLUNTEER, rank: 3, tags: [], 
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
                    <ProfileFormFields 
                        ...
                        contactEmail={formData.id?.includes('@') ? formData.id : ''}
                        setContactEmail={() => {}} // Read-only for admin in this quick editor
                        gender={formData.gender || Gender.MALE} setGender={val => setFormData({...formData, gender: val})}
                        primaryPhone={formData.phone || ''} setPrimaryPhone={val => setFormData({...formData, phone: val})}
                        isSkipper={formData.isSkipper || false} setIsSkipper={val => setFormData({...formData, isSkipper: val})}
                    />
